...
public class HouseService {
     ...
     * @param query
     */
    public PageInfo<House> queryHouse(House query) {
        ...
        List<House> houses = houseMapper.selectPageHouses(query);
        //添加图片服务器地址前缀
        houses = queryAndSetImg(houses);
        PageInfo pageInfo = new PageInfo(houses);
        return pageInfo;
    }

    public List<House> queryAndSetImg(List<House> houses) {
        houses.forEach(h -> {
    ...
    public House queryOneHouse(Long id) {
        ...
        query.setId(id);
        List<House> houses = queryAndSetImg(houseMapper.selectPageHouses(query));
        if (!houses.isEmpty()) {
            return houses.get(0);
        }
        return null;
